...
enum MenuAPI {
    case addressInfo(leadUUID: String)
...
extension MenuAPI: TargetType {
    ...
    var path: String {
        switch self {
        case let .addressInfo(leadUUID): return "/orders/\(leadUUID)/show/"
    ...
    var task: Task {
        switch self {
        case .addressInfo: return .requestPlain
